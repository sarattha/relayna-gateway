...
Start from `deploy/kubernetes/relayna-gateway.yaml`, set the image to your published `0.0.2` image, and provide `relayna-gateway-secrets` through your cluster secret manager. Keep the control port private unless it is protected by an internal ingress, VPN, or identity-aware proxy.
