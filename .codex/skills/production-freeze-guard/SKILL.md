...
description: Use after the Relayna Gateway v0.0.14 production freeze when adding features or changing public routes, APIs, schemas, config, auth, policy, usage, proxy, streaming, Redis, telemetry, admin UI, or release behavior; requires compatibility review and freeze perimeter tests.
---
...
Relayna Gateway v0.0.14 is the production freeze baseline. Use this skill before
...
keeps the v0.0.14 perimeter tests honest.

## Workflow

1. Establish the baseline.
   - Run `git tag -l 'v*' --sort=-v:refname | head -n1`.
   - The expected production freeze baseline is `v0.0.14`.
   - Compare risky changes with `git diff v0.0.14...HEAD -- <path>` when needed.
...
4. Update the freeze perimeter tests.
   - Run `node tests/freeze-v0.0.14-perimeter.test.mjs` before editing to see
...
5. Verify before handoff.
   - Run `node tests/freeze-v0.0.14-perimeter.test.mjs`.
