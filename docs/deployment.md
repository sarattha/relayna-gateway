...
Version `0.1.8` keeps that deployment shape and includes LiteLLM
...
controls for those front-door auth settings. It also adds Admin portal controls
for LiteLLM custom credential headers and Relayna key/project LiteLLM virtual
key mapping. Admin UI 2.0 is compiled into the same static asset contract, and
provider intelligence, operator governance, usage analytics, audit, Entra,
Apigee, and LiteLLM credential controls remain on the same gateway binary. See
...
1. Use the image published by the tag-based release workflow:
   ...
   ```bash
   export RELAYNA_GATEWAY_IMAGE="<your-registry>/<your-org>/relayna-gateway:0.1.8"
...
2. Update the Deployment image when you use a different registry or tag:
   ...
   ```yaml
   image: <your-registry>/<your-org>/relayna-gateway:0.1.8
   ```
