...
Version `0.1.16` keeps that deployment shape and includes bearer-prefixed
...
```bash
docker build -t relayna-gateway:0.1.16 .
```
...
1. Use the image published by the tag-based release workflow:
   ...
   ```text
   ghcr.io/sarattha/relayna-gateway:0.1.16
   ```

   To build and publish manually to another registry:

   ```bash
   export RELAYNA_GATEWAY_IMAGE="<your-registry>/<your-org>/relayna-gateway:0.1.16"
...
2. Update the Deployment image when you use a different registry or tag:
   ...
   ```yaml
   image: <your-registry>/<your-org>/relayna-gateway:0.1.16
   ```
