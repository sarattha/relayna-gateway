...
| Gitleaks `relayna-live-token` in OpenAPI pricing documentation | Relayna Gateway maintainers | `op_live_replace_with_secret_value` is an explicit non-secret placeholder already covered by `.gitleaks.toml`; Gitleaks 8.24.3 still flags its commit diff, so CI ignores only the reported commit fingerprint. | https://github.com/sarattha/relayna-gateway/pull/96 | 2026-08-31 |
