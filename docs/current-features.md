...
scopes and roles. Traffic and Usage show session measurements and verified
identity details. See [Accessa channels](accessa-channels.md).
...
Virtual keys and key-filtered Usage & cost show LiteLLM-reported spend with
mapping scope and retrieval time, keeping shared/resettable counters separate
from Gateway estimates and budgets. See [spend and reliability details](admin-portal.md#overview-reliability-and-litellm-reported-spend).

Gateway-managed requests support opt-in **Require unverified bearer
(troubleshooting)** in Admin Settings. It keeps both client headers and validates
the Relayna key while pausing Entra verification; the bearer establishes no
identity. Clearing the option restores saved Entra verification. See [setup and
restoration](entra-id-auth.md#temporarily-pause-verification-while-keeping-both-headers).

The Admin UI includes shared request investigation with exact internal
...
Administrators and registered service or project owners can sign in with Microsoft Entra
...
client secret. The same Entra application is also the API resource: v2 workload
tokens use its application ID as the exact audience, while managed identities
request `api://<application-id>/.default` and receive only `gateway.invoke` or
...
and exports. Server-side membership and workload-binding checks enforce the
same boundary for `/owner/v1/services/{service_name}/*` and
`/owner/v1/projects/{project_id}/*` APIs.

Managed identities reuse `gateway.monitor.read` for both resource types. The
Entra role grants only the coarse read-only capability; an enabled exact
Relayna service or project binding determines what the token can read. Project
queries use persisted usage-event project attribution rather than service
links, preventing shared services from crossing project boundaries.
...
### Entra authorization incident diagnostics

Release `v0.1.29` adds the opt-in `ENTRA_AUTH_DEBUG` structured decision trail
for direct access tokens, trusted Apigee identities, portal OIDC and cookie
sessions, and owner managed identities. It identifies the first rejected
discovery, JWKS, signature, claim, role, Relayna binding, database-session,
cookie-emission, browser-return, or CSRF phase. Decoded claims are available
only while the sensitive mode is explicitly enabled; OAuth transaction claims
are redacted, and compact tokens, codes, PKCE material, cookie values/hashes,
and raw Relayna keys are never logged. See
