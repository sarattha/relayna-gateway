...
Relayna Gateway uses `vMAJOR.MINOR.PATCH` Git tags. Version `0.0.8` is the current release target.
...
3. Run the full verification stack:
   ...
   ```bash
   python3 scripts/validate-release-metadata.py v0.0.8
   cargo fmt --all --check
...
For `v0.0.8`, the workflow publishes:
