...
Relayna Gateway uses `vMAJOR.MINOR.PATCH` Git tags. Version `0.1.18` is the current release target.

Version `0.1.18` includes Admin UI 2.0, operator governance, policy
governance, provider intelligence, observability analytics, supply-chain
hardening, structured service pricing-rule controls, paginated usage analytics
tables, LiteLLM
`/v1/embeddings` passthrough, opt-in Entra ID and Apigee front-door
...
3. Run the full verification stack:
   ...
   ```bash
   python3 scripts/validate-release-metadata.py v0.1.18
   cargo fmt --all --check
...
For `v0.1.18`, the workflow publishes:
