...
Relayna Gateway uses `vMAJOR.MINOR.PATCH` Git tags. Version `0.0.4` is the current release target.
...
3. Run the full verification stack:
   ...
   ```bash
   python3 scripts/validate-release-metadata.py v0.0.4
   cargo fmt --all --check
...
For `v0.0.4`, the workflow publishes:
