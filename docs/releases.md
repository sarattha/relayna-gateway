...
Relayna Gateway uses `vMAJOR.MINOR.PATCH` Git tags. Version `0.1.16` is the current release target.

Version `0.1.16` includes Admin UI 2.0, operator governance, policy
...
3. Run the full verification stack:
   ...
   ```bash
   python3 scripts/validate-release-metadata.py v0.1.16
   cargo fmt --all --check
...
For `v0.1.16`, the workflow publishes:
