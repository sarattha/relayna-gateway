...
Relayna Gateway uses `vMAJOR.MINOR.PATCH` Git tags. Version `0.1.17` is the current release target.

Version `0.1.17` includes Admin UI 2.0, operator governance, policy
governance, provider intelligence, observability analytics, supply-chain
hardening, structured service pricing-rule controls, LiteLLM
...
3. Run the full verification stack:
   ...
   ```bash
   python3 scripts/validate-release-metadata.py v0.1.17
   cargo fmt --all --check
...
For `v0.1.17`, the workflow publishes:
